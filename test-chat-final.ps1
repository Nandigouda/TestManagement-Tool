# Chat Functionality Test Script with UseBasicParsing
...
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8081" -ErrorAction SilentlyContinue -UseBasicParsing -TimeoutSec 5
    Write-Status "OK: Server is running on port 8081" "Green"
} catch {
    Write-Status "ERROR: Server is NOT responding" "Red"
    exit 1
}
...
if ($conversationId) {
    ...
    try {
        ...
        Write-Host "  - Response content: $($result.content.Substring(0, 80))..."
    } catch {
...
if ($conversationId) {
    ...
    try {
        ...
        if ($result -is [System.Collections.IEnumerable]) {
            Write-Host "  - Found $($result.Count) results"
        }
    } catch {
...
Write-Host ""
Write-Host "Summary:"
Write-Host "- POST /chat/start - Start new conversation"
Write-Host "- GET /chat/{id} - Get conversation details"
Write-Host "- POST /chat/message - Send message to conversation"
Write-Host "- POST /chat/{id}/regenerate - Regenerate test cases"
Write-Host "- POST /chat/{id}/modify - Modify test cases"
Write-Host "- POST /chat/{id}/add-more - Add more test cases"
Write-Host "- POST /chat/{id}/merge - Merge duplicate test cases"
Write-Host "- GET /chat/{id}/similar - Find similar test cases"
